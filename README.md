...
- **Visit Destination (Tujuan Kunjungan)**: SPV daily tasks display the visit destination store (branch code + name); the SPV dashboard shows a prominent "Tujuan Kunjungan" banner on the Daily Task card
- **Manager SPV Kanban — Grouped per SPV**: The HR and Operational dashboards group the "Task SPV Kanban" panel into compact one-per-row cards per SPV (name, team, destination, `verified/total` badge); clicking a card opens a modal listing that SPV's tasks, and clicking a task opens the full detail modal
...
- **Daily Reporters API**: Public endpoint at `GET /api/reports/daily-reporters?date=YYYY-MM-DD` returns the daily report feed for every position required to report (any position with a report-field template, not just managers). Returns `reports` (submitted report fields only — no KPI task details) and `pending` (not yet submitted); `date` defaults to yesterday
