...
function TypeBadge({ type }: { type: DocType }) {
    if (type === 'folder') {
        return <span className="text-xs font-medium text-amber-600 dark:text-amber-400">Folder</span>;
    }
    if (type === 'document') {
        return <span className="text-xs font-medium text-blue-600 dark:text-blue-400">Dokumen</span>;
    }
...
export function DokumenTab({ team }: { team: any }) {
    const { auth } = usePage<{ auth: { user: { id: number }; roles: string[] } }>().props;
    const currentUserId = auth?.user?.id;
    const isAdmin = auth?.roles?.some(r => ['superadmin', 'admin'].includes(r));
    ...
    const fileForm = useForm({ file: null as File | null, parent_id: null as string | null });
    const renameForm = useForm({ id: '', name: '' });
    const updateFileForm = useForm({ id: '', file: null as File | null });

    const fetchDocuments = useCallback(async (parentId: string | null = null) => {
        setIsLoading(true);
        try {
            const url = new URL(`/teams/${team.slug}/documents`, window.location.origin);
    ...
    const handleCreateFolder = (e: React.FormEvent) => {
        e.preventDefault();
        folderForm.post(`/teams/${team.slug}/documents/folder`, {
            onSuccess: () => {
    ...
    const handleUploadFile = (e: React.FormEvent) => {
        e.preventDefault();
        fileForm.post(`/teams/${team.slug}/documents/file`, {
            preserveScroll: true,
            forceFormData: true,
            onSuccess: () => {
    ...
    const handleDelete = (docId: string) => {
        if (confirm('Yakin ingin menghapus item ini?')) {
            router.delete(`/teams/${team.slug}/documents/${docId}`, {
                preserveScroll: true,
    ...
    const handleUpdateFile = (e: React.FormEvent) => {
        e.preventDefault();
        updateFileForm.post(`/teams/${team.slug}/documents/${updateFileForm.data.id}/update-file`, {
            preserveScroll: true,
            forceFormData: true,
            onSuccess: () => {
                setIsUpdateFileModalOpen(false);
                updateFileForm.reset('file', 'id');
                fetchDocuments(currentParentId);
            },
        });
    };
    ...
    return (
        <div className="flex h-full flex-col">
            ...
            <div className="flex shrink-0 items-center justify-between gap-4 border-b border-slate-100 px-6 py-4 dark:border-slate-800">
                ...
                <nav className="no-scrollbar flex flex-1 items-center gap-1 overflow-x-auto text-sm">
                    ...
                    {breadcrumbs.map((crumb) => (
                        <div key={crumb.id} className="flex shrink-0 items-center gap-1 text-slate-400">
                ...
                <div className="flex shrink-0 items-center gap-2">
                    ...
                    <Button
                        ...
                        className="h-9 rounded-lg px-3 text-slate-600 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-zinc-800"
                    >
                        <Link href={`/teams/${team.slug}/documents/create${currentParentId ? `?parent_id=${currentParentId}` : ''}`}>
                ...
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-32 text-center">
                        ...
                        <p className="max-w-xs text-sm text-slate-400">
                            Mulai dengan membuat folder, menulis dokumen, atau mengunggah file.
                        </p>
                        <div className="mt-6 flex gap-3">
                            ...
                            >
                                <FolderPlus className="mr-1.5 h-4 w-4" /> Buat Folder
                            </Button>
                            <Button size="sm" asChild>
                                <Link href={`/teams/${team.slug}/documents/create`}>
                                    <FilePlus className="mr-1.5 h-4 w-4" /> Buat Dokumen
                                </Link>
                            </Button>
                        </div>
                    </div>
                    ...
                    <div className="p-4">
                        {/* List header */}
                        <div className="mb-1 grid grid-cols-[1fr_auto_auto_auto] items-center gap-4 px-4 py-2 text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">
                            <span>Nama</span>
                            <span className="hidden w-24 text-right sm:block">Tipe</span>
                            <span className="hidden w-24 text-right md:block">Diperbarui</span>
                            <span className="w-8" />
                        </div>
                            ...
                            {sorted.map((doc) => (
                                ...
                                >
                                    ...
                                    <div className="flex min-w-0 items-center gap-3">
                                        <FileIcon2 type={doc.type} name={doc.name} />
                                        <div className="min-w-0">
                                            {doc.type === 'folder' ? (
                                                ...
                                                <Link
                                                    href={`/teams/${team.slug}/documents/${doc.id}`}
                                                    className="block truncate text-sm font-semibold text-slate-800 transition-colors hover:text-primary dark:text-slate-100"
                                                >
                                                    {doc.name}
                                                </Link>
                                            ...
                                            <div className="mt-0.5 flex items-center gap-1.5 truncate text-xs text-slate-400">
                                                {doc.user && (
                                                    <Avatar className="h-4 w-4">
                                                        <AvatarImage src={doc.user.avatar_url ?? undefined} />
                                                        <AvatarFallback className="text-[8px]">{doc.user.name.charAt(0)}</AvatarFallback>
                                                    </Avatar>
                                                )}
                                                <span>
                                                    {doc.user?.name}
                                                    {doc.type === 'file' && doc.media?.[0] && (
                                                        <> · {formatSize(doc.media[0].size)}</>
                                                    )}
                                                </span>
                                            </div>
                                        </div>
                                    ...
                                    {/* Actions col */}
                                    <div className="w-8">
                                        {(doc.user_id === currentUserId || isAdmin) && (
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end" className="w-44 rounded-xl">
                                                <DropdownMenuItem
                                                    onClick={() => {
                                                        renameForm.setData({ id: doc.id, name: doc.name });
                                                ...
                                                {doc.type === 'document' && (
                                                    <DropdownMenuItem asChild className="cursor-pointer">
                                                        <Link href={`/teams/${team.slug}/documents/${doc.id}/edit`}>
                                                ...
                                                {doc.type === 'file' && (
                                                    <>
                                                        <DropdownMenuItem
                                                            onClick={() => {
                                                                updateFileForm.setData('id', doc.id);
                                                        ...
                                                        <DropdownMenuItem asChild className="cursor-pointer">
                                                            <a href={doc.media?.[0]?.original_url} download>
            ...
            <Dialog open={isFolderModalOpen} onOpenChange={setIsFolderModalOpen}>
                <DialogContent className="rounded-2xl">
                    <form onSubmit={handleCreateFolder}>
                        ...
                        <DialogFooter>
                            <Button type="button" variant="outline" onClick={() => setIsFolderModalOpen(false)}>Batal</Button>
                            <Button type="submit" disabled={folderForm.processing}>Buat</Button>
                        </DialogFooter>
                    </form>
                </DialogContent>
            </Dialog>
                    ...
                    <form onSubmit={handleUploadFile}>
                        ...
                        <div className="py-4">
                            ...
                            <Input
                                ...
                                className="mt-2"
                            />
                            <p className="mt-2 text-xs text-muted-foreground">Maks 10MB. Format: pdf, doc, docx, xls, xlsx, png, jpg, jpeg.</p>
                        </div>
                        <DialogFooter>
                            <Button type="button" variant="outline" onClick={() => setIsFileModalOpen(false)}>Batal</Button>
                            <Button type="submit" disabled={fileForm.processing}>Upload</Button>
                        </DialogFooter>
                    </form>
                </DialogContent>
            </Dialog>
            ...
            <Dialog open={isRenameModalOpen} onOpenChange={setIsRenameModalOpen}>
                <DialogContent className="rounded-2xl">
                    <form
                        onSubmit={(e) => {
                            e.preventDefault();
                            renameForm.put(`/teams/${team.slug}/documents/${renameForm.data.id}`, {
                                preserveScroll: true,
                                onSuccess: () => {
                    ...
                    >
                        ...
                        <DialogFooter>
                            <Button type="button" variant="outline" onClick={() => setIsRenameModalOpen(false)}>Batal</Button>
            ...
            <Dialog open={isUpdateFileModalOpen} onOpenChange={setIsUpdateFileModalOpen}>
                <DialogContent className="rounded-2xl">
                    <form onSubmit={handleUpdateFile}>
                        ...
                        <div className="py-4">
                            <p className="mb-4 text-sm text-slate-500">
                                Unggah file pengganti. File lama akan tetap tersimpan sebagai riwayat (History).
                            </p>
                            <Label htmlFor="update_file_upload">Pilih File Baru</Label>
                            <Input
                                id="update_file_upload"
                                type="file"
                                onChange={(e) => updateFileForm.setData('file', e.target.files?.[0] || null)}
                                required
                                className="mt-2"
                            />
                            {updateFileForm.errors.file && (
                                <p className="mt-2 text-xs text-red-500">{updateFileForm.errors.file}</p>
                            )}
                        </div>
                        <DialogFooter>
                            <Button type="button" variant="outline" onClick={() => setIsUpdateFileModalOpen(false)}>Batal</Button>
                            <Button type="submit" disabled={updateFileForm.processing}>
                                {updateFileForm.processing && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                                Unggah
                            </Button>
                        </DialogFooter>
                    </form>
